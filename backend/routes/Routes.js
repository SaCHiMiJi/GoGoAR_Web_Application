...
app.post("/create", verifyToken, async (req, res) => {
  ...
  assignment_repository
    ...
    .then((createassignment) => {
      ...
      assignment_repository
        .updateURL(id, "unitydl://mylink?" + id)
        .then((updatedDocument) => {
          if (!updatedDocument) {
            // If no assignment was updated, send a 404 response
            return res
              .status(404)
              .json({ message: "No assignment found with the given ID" });
          }
...
app.get("/getassignmentbyuser/:id", async (req, res) => {
  ...
  assignment_repository
    .findByCreatorID(userid.toString())
    .then((assignments) => {
      return res.json(assignments);
    })
    .catch((error) => {
...
app.post("/getassignmentgroup", async (req, res) => {
  try {
    ...
    if (!Array.isArray(assignmentIds)) {
      return res
        .status(400)
        .json({ message: "Input should be an array of assignment IDs" });
    }
    ...
    res
      .status(500)
      .json({ message: "Error retrieving assignments", error: error });
  }
});
...
app.get("/getqrcode/:id", async (req, res) => {
  ...
  try {
    ...
    const url = dburl.mobileapp_url;
    try {
      const qrcode_img = await QRCode.toDataURL(
        process.env.FRONTEND_URL + "/appredirection?url=" + url,
      );
...
app.post("/register", async (req, res) => {
  try {
    ...
    creator_repository.create(creatorDetail).then(() => {
...
app.post("/login", async (req, res) => {
  ...
  if (!passwordMatch) {
    return res
      .status(401)
      .json({ error: "Wrong password, Please tried again." });
  }
...
app.post("/forgotpassword", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Error occurred while deleting OTP:", error);
...
app.get("/getcreatorname/:id", (req, res) => {
  ...
  if (id === process.env.EMBEDDED_USER) {
    return res.send("Go Go Board AR");
  }
